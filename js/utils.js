...
utils.hexToLittleEndian = function (hex_string) {
    if (hex_string.length % 2 == 1) {
        console.log("ERROR: invalid hex string passed to hexToLittleEndian");
        return hex_string;
    }
    le_hex = "";
    var i = 0
    while (i < hex_string.length) {
        le_hex = hex_string.substring(i, i + 2) + le_hex;
        i = i + 2;
    }
    return le_hex;
}
...
utils.deltaLevelToHex = function (delta) {
    var hex_digits = "0123456789ABCDEF";

    var the_answer = "";

    for (var i = 8; i >= 0; i--) {
        the_answer = hex_digits.charAt(delta & 0xF) + the_answer;
        delta >>= 4;
    }

    return the_answer.substring(1, 9);
}